...
// Lightweight Supabase client (no npm needed - works via CDN in Vite)
let _supabase = null;
function getSupabase() {
  if (_supabase) return _supabase;
  const { createClient } = window.supabase || {};
  if (createClient) { _supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY); return _supabase; }
  return null;
}
...
:root{
  --bg:#0a0c10;--bg2:#111318;--bg3:#181b22;--bg4:#1e2230;
  --border:#1e2230;--border2:#252a3a;
  --accent:#00d4a8;--accent2:#0099ff;--accent3:#ff6b35;--accent4:#a855f7;
  --text:#e8eaf0;--text2:#8a8fa8;--text3:#50556a;
  --danger:#ff4466;--warn:#ffaa00;--success:#00d4a8;
...
.nav-item.active{background:rgba(0,212,168,.12);color:var(--accent)}
...
.avatar{width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--accent2));display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;color:#000;flex-shrink:0}
...
td{padding:10px 12px;font-size:13px;border-bottom:1px solid rgba(30,34,48,.5);vertical-align:middle}
...
.bg{background:rgba(0,212,168,.12);color:var(--accent)}
...
/* ── Modals ── */
.overlay{position:fixed;inset:0;background:rgba(0,0,0,.75);display:flex;align-items:center;justify-content:center;z-index:200;padding:20px;backdrop-filter:blur(4px)}
.modal{background:var(--bg2);border:1px solid var(--border);border-radius:16px;width:100%;max-height:92vh;overflow-y:auto;box-shadow:0 24px 60px rgba(0,0,0,.6)}
...
/* ── Login ── */
.login-wrap{min-height:100vh;display:flex;align-items:center;justify-content:center;background:var(--bg);background-image:radial-gradient(ellipse at 20% 50%,rgba(0,212,168,.05) 0%,transparent 50%),radial-gradient(ellipse at 80% 20%,rgba(0,153,255,.05) 0%,transparent 50%)}
...
.login-card{background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:28px;box-shadow:0 24px 60px rgba(0,0,0,.4)}
...
.ltab.active{background:var(--bg2);color:var(--accent);box-shadow:0 2px 8px rgba(0,0,0,.3)}
...
export default function App() {
  ...
  const [stockTakes, setStockTakes] = useState(DEMO_STOCK_TAKES);
  ...
  // ── Demo login ──
  const login = (u) => { setUser(u); setPage(u.role==="admin"?"dashboard":"catalog"); };
  const logout = () => { setUser(null); setCart([]); };

  if (!user) return <><style dangerouslySetInnerHTML={{__html:STYLES}}/><LoginPage customers={customers} setCustomers={setCustomers} onLogin={login}/></>;
  ...
  const placeOrder = (payMethod, notes, orderType, consignmentDue) => {
    const customer = customers.find(c=>c.id===user.id)||{company:user.name,tax_id:""};
    ...
    const o = { id, customer_id:user.id, customer_name:customer.company||user.name, date:today(),
      status:"pending", payment_method:payMethod, subtotal:cartSubtotal, tax_rate:settings.tax_rate,
      tax_amount:cartTax, total:cartTotal, notes, type:orderType,
      consignment_due:consignmentDue||null,
      items:cart.map(i=>({product_id:i.pid,name:i.name,qty:i.qty,unit_price:i.price,barcode:i.barcode||"—",sku:i.sku}))
    };
...
// ─── LOGIN PAGE ───────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
function LoginPage({ customers, setCustomers, onLogin }) {
  ...
  const doLogin = () => {
    setErr("");
    if (email===ADMIN.email && password===ADMIN.password) { onLogin(ADMIN); return; }
    const c = customers.find(c=>c.email===email && c.password===password);
    if (!c) { setErr("Invalid email or password."); return; }
    if (!c.approved) { setErr("Your account is pending approval. Please check back soon."); return; }
    onLogin({...c, role:"buyer"});
  };

  const doRegister = () => {
    setErr("");
    if (!reg.name||!reg.company||!reg.taxId||!reg.email||!reg.password) { setErr("All fields required."); return; }
      ...
      <div className="login-box">
        ...
        <div className="login-card">
          ...
          {tab==="register"&&!success&&<>
            ...
            <button className="btn btn-primary" style={{width:"100%",justifyContent:"center"}} onClick={doRegister}>Submit Application →</button>
          </>}
        </div>
      </div>
    </div>
